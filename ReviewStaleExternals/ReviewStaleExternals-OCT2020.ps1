...
function Find-AzureADStaleExternals($authHeaders, $staleDays=180, $createReviewGroups=$false, $scheduleReviews=$false, $JSONPath = "C:\temp\CreateJSON.json")
{
    ...
    if($scheduleReviews)
    {
        Create-AzureADARScheduleDefinition $authHeaders $JSONPath $neverSignedInGroupObjectID "never"
        Create-AzureADARScheduleDefinition $authHeaders $JSONPath $beyondCutOffDAysGroupObjectID "beyond"
    }
}
...
function Create-AzureADARScheduleDefinition($authHeaders, $JSONPath, $groupObjectID, $groupType)
{
    ...
    switch ($groupType)
    {
        "never" 
        {
            $createJSON = $createJSON.Replace("<<MYREVIEW>>", "Review of external identitiesthat have never signed in.")
...
Connect-AzureADMSARSample -ClientApplicationId "ABCD" -ClientSecret "OADDR" -TenantDomain "yourtenant.onmicrosoft.com"
