...
function Find-AzureADStaleExternals($authHeaders, $staleDays=180, $createReviewGroups=$false, $scheduleReviews=$false, $JSONPath = "C:\temp\ARtemplate.json")
{
    ...
    $pendingAcceptanceGracePeriodInDays = 30
    $pendingAcceptanceCutOffDate = (Get-Date (Get-Date).AddDays(-$pendingAcceptanceGracePeriodInDays) -Format s) + "Z"

    ##This is the Graph Call for getting all external identities. 
    $listURL = 'https://graph.microsoft.com/beta/users?$select=id,displayName,userprincipalname,userType,signInActivity,externalUserState,externalUserStateChangeDateTime&$filter=userType eq ''Guest'''
    ...
    foreach($d in $data)
    {
        ...
        {
            #Now that we're here, we know the person has not signed into our tenant at all. Let's catch one corner case: the person was *just* invited
            #to the tenant and did not have a chance to accept the invitation. In that case, let's NOT add them to the AR group, but give them time to
            #accept the invite:
            if(($d.externalUserState -eq "PendingAcceptance") -and ($d.externalUserStateChangeDateTime -gt $pendingAcceptanceCutOffDate))
            {
                Continue;
            }
...
Connect-AzureADMSARSample -ClientApplicationId "<appID>" -ClientSecret "<cliendSecret>" -TenantDomain "<tenantname>.onmicrosoft.com"
